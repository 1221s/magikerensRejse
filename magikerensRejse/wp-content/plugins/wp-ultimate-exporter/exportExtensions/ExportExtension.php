...
class ExportExtension {
	...
	public function FetchDataByPostTypes ($mode = null) {
		...
		if(!empty($recordsToBeExport)) {
			foreach($recordsToBeExport as $postId) {
				...
				if($exp_module == 'Posts' || $exp_module =='WooCommerce' || $exp_module == 'CustomPosts' || $exp_module == 'Categories' || $exp_module == 'Tags' || $exp_module == 'Taxonomies'){
	...
	public function getToolSetRelationshipFieldValue($post_id, $module){

		global $wpdb;
		$toolset_fieldvalues = [];
		$toolset_fieldvalues['intermediate'] = $toolset_fieldvalues['types_relationship'] = null;
		$child_rel='';

		if($module == 'CustomPosts'){
			$toolset_fieldvalues['intermediate'] = $this->getToolSetIntermediateFieldValue($post_id);
		}else if($module == 'Post'){
			$toolset_fieldvalues['types_relationship'] = $this->getToolSetTypeRelationshipFieldValue($post_id);
		}

		$toolset_fieldvalues['relationship_slug'] = $this->getToolSetRelationshipSlugValue($post_id);

		$child_post = "select parent_id,post_title from {$wpdb->prefix}toolset_associations INNER JOIN {$wpdb->prefix}posts on {$wpdb->prefix}posts.ID ={$wpdb->prefix}toolset_associations.parent_id WHERE {$wpdb->prefix}toolset_associations.child_id={$post_id} AND post_status = 'publish'";

		$child_title = $wpdb->get_results($child_post,ARRAY_A);
		//Added for types child relationship export
		foreach($child_title as $key=>$value)
		{
			$child_rel = $value['post_title'];
			$toolset_fieldvalues['types_relationship']=$child_rel;
			$types_id=$wpdb->get_row("select relationship_id from {$wpdb->prefix}toolset_associations where child_id ='{$post_id}'");
			if(!empty($types_id)){
				$result_slug=$wpdb->get_row("select slug from {$wpdb->prefix}toolset_relationships where  id='{$types_id->relationship_id}'");
				if(!empty($result_slug)){
					$toolset_fieldvalues['relationship_slug']=$result_slug->slug;
				}else{
					$toolset_fieldvalues['relationship_slug']='';	
				}
			}else{
				$toolset_fieldvalues['relationship_slug']='';
			}
		...
		//Added for types parent relationship export
		foreach($parent_title as $key1=>$value1)
		{
			if(is_array($value1)){
				$post_title .= $value1['post_title'].',';
				$rel_title=rtrim($post_title,","); 
				$relationship_title=$rel_title.'|';
			}                       
			$parent_rel =  $relationship_title ;
			$parent_relationship=rtrim($parent_rel,"| ");   
			$toolset_fieldvalues['types_relationship']= $parent_relationship;
			$get_slug = "select relationship_id from {$wpdb->prefix}toolset_associations where parent_id ='{$post_id}'";
			$rel_slug=$wpdb->get_results($get_slug,ARRAY_A);

			foreach($rel_slug as $relkey=>$relvalue)
			{
				$relationship_id = $relvalue['relationship_id'];
				$slug_id="select slug from wp_toolset_relationships where  id IN ($relationship_id)";
				$relationship=$wpdb->get_results($slug_id,ARRAY_A);

				foreach($relationship as $keys=>$values) {
					//$slug = $values['slug'];
					$toolset_fieldvalues['relationship_slug']=$values['slug'];
				}
			}
		}

		$both=$parent_relationship.'|'.$child_rel;$str = ltrim($both, '|');
		$toolset_fieldvalues['types_relationship']=rtrim($str,"|");
		$both1=$slug.'|'.$result_slug->slug;$str1 = ltrim($both1, '|');
		$toolset_fieldvalues['relationship_slug']=rtrim($str1,"|");
		//Added for types parent relationship export

		$intermediate_rel=$wpdb->get_var("select relationship_id from {$wpdb->prefix}toolset_associations where intermediary_id ='{$post_id}'");

		$intermediate_slug=$wpdb->get_var("select slug from wp_toolset_relationships where  id IN ($intermediate_rel)");
		...
		if(!empty($intermediate_slug)){
			...
			$intermediate_post = "select parent_id,child_id,post_title from {$wpdb->prefix}toolset_associations INNER JOIN {$wpdb->prefix}posts on {$wpdb->prefix}posts.ID =wp_toolset_associations.child_id WHERE {$wpdb->prefix}toolset_associations.intermediary_id='{$post_id}' AND post_status = 'publish'";
			...
			foreach($related_ids as $keyd=>$valued)
			{
				...
				if(!empty($parent_id)){
					$related_posts = $wpdb->get_var("select post_title from {$wpdb->prefix}posts where ID = $parent_id AND post_status = 'publish'",ARRAY_A);
				}
				if(!empty($child_id)){
